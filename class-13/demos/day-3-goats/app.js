...
function handleClickOnBothGoats(event){
  ...
  if(likeCounter > 9){
    ...
    // Save the goat data;
    console.log('saving goats to local storage');
    var stringyGoats = JSON.stringify(allGoats);
    localStorage.setItem('allGoats', stringyGoats);
  }
...
if(localStorage.getItem('allGoats') === null){
  // Instantiate GoatImage objects
  console.log('making new goats');
  ...
  console.log('retrieving goats from local storage');
  var stringyGoats = localStorage.getItem('allGoats');
  allGoats = JSON.parse(stringyGoats);
}
